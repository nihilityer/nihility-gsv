use crate::error::{NihilityGsvError, Result};
use std::sync::Arc;
...
impl T2S {
    ...
    pub fn new(file_path: &str, device: Device) -> Result<Self> {
        let mut model = tch::CModule::load_on_device(file_path, device)?;
        model.set_eval();
    ...
    pub fn pre_infer(
        ...
        text_bert: Tensor,
        top_k: i64,
    ) -> Result<(i64, Tensor, Tensor, Vec<Tensor>, Vec<Tensor>)> {
        let result = self.model.method_is(
            "pre_infer",
            &[
        ...
        if let IValue::Tuple(mut result) = result {
            let v_cache = result.pop().ok_or(NihilityGsvError::Infer(
                "Take v_cache from T2S model pre-inference result failed".to_string(),
            ))?;
            let k_cache = result.pop().ok_or(NihilityGsvError::Infer(
                "Take k_cache from T2S model pre-inference result failed".to_string(),
            ))?;
            let xy_pos = result.pop().ok_or(NihilityGsvError::Infer(
                "Take xy_pos from T2S model pre-inference result failed".to_string(),
            ))?;
            let y = result.pop().ok_or(NihilityGsvError::Infer(
                "Take y from T2S model pre-inference result failed".to_string(),
            ))?;
            let y_len = result.pop().ok_or(NihilityGsvError::Infer(
                "Take y_len from T2S model pre-inference result failed".to_string(),
            ))?;
            ...
            ) = (y_len, y, xy_pos, k_cache, v_cache)
            {
                ...
                Err(NihilityGsvError::Infer(
                    "Unexpected types in T2S model pre-inference result".to_string(),
                ))
            }
        } else {
            Err(NihilityGsvError::Infer(
                "Expected a tuple from T2S model pre-inference result".to_string(),
            ))
        }
    }
    ...
    pub fn decode_next_token(
        ...
        k_cache: Vec<Tensor>,
        v_cache: Vec<Tensor>,
    ) -> Result<(Tensor, Tensor, i64, Vec<Tensor>, Vec<Tensor>)> {
        ...
        if let IValue::Tuple(mut result) = result {
            let v_cache = result.pop().ok_or(NihilityGsvError::Infer(
                "Take v_cache from T2S model decode result failed".to_string(),
            ))?;
            let k_cache = result.pop().ok_or(NihilityGsvError::Infer(
                "Take k_cache from T2S model decode result failed".to_string(),
            ))?;

            let last_token = result.pop().ok_or(NihilityGsvError::Infer(
                "Take last_token from T2S model decode result failed".to_string(),
            ))?;
            let xy_pos = result.pop().ok_or(NihilityGsvError::Infer(
                "Take xy_pos from T2S model decode result failed".to_string(),
            ))?;
            let y = result.pop().ok_or(NihilityGsvError::Infer(
                "Take y from T2S model decode result failed".to_string(),
            ))?;
            ...
            ) = (y, xy_pos, last_token, k_cache, v_cache)
            {
                ...
                Err(NihilityGsvError::Infer(
                    "Unexpected types in T2S model decode result".to_string(),
                ))
            }
        } else {
            Err(NihilityGsvError::Infer(
                "Expected a tuple from T2S model decode result".to_string(),
            ))
        }
    }
}
...
impl Vits {
    ...
    /// return: (refer, sv_emb)
    pub fn ref_handle(&self, ref_audio_32k: Tensor) -> Result<(Tensor, Tensor)> {
        let result = self
            .model
            .method_is("ref_handle", &[&IValue::Tensor(ref_audio_32k)])?;
        if let IValue::Tuple(mut result) = result {
            // refer, sv_emb
            let sv_emb = result.pop().ok_or(NihilityGsvError::Infer(
                "Take sv_emb from VITS model ref_handle result failed".to_string(),
            ))?;
            let refer = result.pop().ok_or(NihilityGsvError::Infer(
                "Take refer from VITS model ref_handle result failed".to_string(),
            ))?;
            ...
            if let (IValue::Tensor(refer), IValue::Tensor(sv_emb)) = (refer, sv_emb) {
                ...
                Err(NihilityGsvError::Infer(
                    "Unexpected types in VITS model ref_handle result".to_string(),
                ))
            }
        } else {
            Err(NihilityGsvError::Infer(
                "Expected a tuple from VITS model ref_handle result".to_string(),
            ))
        }
    }

    pub fn extract_latent(&self, ssl_content: Tensor) -> Result<Tensor> {
        let r = self.model.method_ts("extract_latent", &[ssl_content])?;
        Ok(r)
    }
...
impl SSL {
    ...
    /// return: ssl_content
    pub fn to_ssl_content(&self, audio_16k: Tensor) -> Result<Tensor> {
        let r = self.ssl.forward_ts(&[audio_16k])?;
        Ok(r)
    }

    pub fn resample(&self, audio: &Tensor, sr: usize, target_sr: usize) -> Result<Tensor> {
        tch::no_grad(|| {
...
impl SpeakerV2Pro {
    ...
    pub fn pre_handle_ref(&self, ref_audio_32k: Tensor) -> Result<(Tensor, Tensor, Tensor)> {
